 ...
 */
class WC_YouCanPay_Pre_Orders_Compat extends WC_YouCanPay_Payment_Gateway {
	public $saved_cards;

	public function __construct() {
		$this->saved_cards = WC_YouCanPay_Helper::get_settings( 'youcanpay', 'saved_cards' );
	}
	...
	public function remove_order_source_before_retry( $order ) {
		...
		$order->delete_meta_data( '_youcanpay_card_id' );
		$order->save();
	}
	...
	public function process_pre_order( $order_id ) {
		try {
			...
			WC_YouCanPay_Logger::log( 'Pre Orders Error: ' . $e->getMessage() );
	...
	public function process_pre_order_release_payment( $order, $retry = true ) {
		try {
			...
			if ( ! empty( $response->error ) && ! $is_authentication_required ) {
				...
				do_action( 'wc_gateway_youcanpay_process_payment_authentication_required', $order );
