...
class WC_YouCanPay_Email_Failed_Authentication_Retry extends WC_Email_Failed_Order {
	...
	public function __construct() {
		...
		$this->title       = __( 'Payment Authentication Requested Email', 'youcan-pay' );
		$this->description = __( 'Payment authentication requested emails are sent to chosen recipient(s) when an attempt to automatically process a subscription renewal payment fails because the transaction requires an SCA verification, the customer is requested to authenticate the payment, and a retry rule has been applied to notify the customer again within a certain time period.', 'youcan-pay' );

		$this->heading = __( 'Automatic renewal payment failed due to authentication required', 'youcan-pay' );
		$this->subject = __( '[{site_title}] Automatic payment failed for {order_number}. Customer asked to authenticate payment and will be notified again {retry_time}', 'youcan-pay' );
		...
		$this->template_base  = plugin_dir_path( WC_YOUCAN_PAY_MAIN_FILE ) . 'templates/';
	...
	public function trigger( $order_id, $order = null ) {
		...
		if ( class_exists( 'WCS_Retry_Manager' ) && function_exists( 'wcs_get_human_time_diff' ) ) {
			...
			WC_YouCanPay_Logger::info( 'WCS_Retry_Manager class or does not exist. Not able to send admin email about customer notification for authentication required for renewal payment.' );
			return;
		}
