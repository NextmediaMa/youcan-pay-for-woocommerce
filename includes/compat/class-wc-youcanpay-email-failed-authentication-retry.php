...
class WC_YouCanPay_Email_Failed_Authentication_Retry extends WC_Email_Failed_Order {
	...
	public function __construct() {
		...
		$this->title       = __( 'Payment Authentication Requested Email', 'youcan-pay-for-woocommerce' );
		$this->description = __( 'Payment authentication requested emails are sent to chosen recipient(s) when an attempt to automatically process a subscription renewal payment fails because the transaction requires an SCA verification, the customer is requested to authenticate the payment, and a retry rule has been applied to notify the customer again within a certain time period.', 'youcan-pay-for-woocommerce' );

		$this->heading = __( 'Automatic renewal payment failed due to authentication required', 'youcan-pay-for-woocommerce' );
		$this->subject = __( '[{site_title}] Automatic payment failed for {order_number}. Customer asked to authenticate payment and will be notified again {retry_time}', 'youcan-pay-for-woocommerce' );
