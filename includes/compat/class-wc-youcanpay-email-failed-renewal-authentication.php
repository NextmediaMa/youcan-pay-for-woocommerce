 ...
 */
class WC_YouCanPay_Email_Failed_Renewal_Authentication extends WC_YouCanPay_Email_Failed_Authentication {
	/**
	 * Constructor.
	 *
	...
	public function __construct( $email_classes = [] ) {
		...
		add_action( 'wc_gateway_youcanpay_process_payment_authentication_required', [ $this, 'trigger' ] );
	...
	public function set_store_owner_custom_email( $rule_array, $retry_number, $order_id ) {
		if (
			...
			$rule_array['email_template_admin'] = 'WC_YouCanPay_Email_Failed_Authentication_Retry';
		}
