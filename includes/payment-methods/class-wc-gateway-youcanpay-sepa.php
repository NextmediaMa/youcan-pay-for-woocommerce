 ...
 * @since 4.0.0
 */
class WC_Gateway_YouCanPay_Sepa extends WC_YouCanPay_Payment_Gateway {

	const ID = 'youcanpay_sepa';
	...
	public function __construct() {
		$this->id           = self::ID;
		$this->method_title = __( 'YouCan Pay SEPA Direct Debit', 'woocommerce-gateway-youcanpay' );
		/* translators: link */
		$this->method_description = sprintf( __( 'All other general YouCan Pay settings can be adjusted <a href="%s">here</a>.', 'woocommerce-gateway-youcanpay' ), admin_url( 'admin.php?page=wc-settings&tab=checkout&section=youcanpay' ) );
	...
	public function get_supported_currency() {
		return apply_filters(
			'wc_youcanpay_sepa_supported_currencies',
			[
				'EUR',
			]
		);
	...
	public function init_form_fields() {
		$this->form_fields = require WC_YOUCAN_PAY_PLUGIN_PATH . '/includes/admin/youcanpay-sepa-settings.php';
	}
	...
	public function form() {
		...
		<fieldset id="wc-<?php echo esc_attr( $this->id ); ?>-form" class="wc-payment-form">
			...
			<p class="wc-youcanpay-sepa-mandate" style="margin-bottom:40px;"><?php $this->mandate_display(); ?></p>
			<p class="form-row form-row-wide">
				<label for="youcanpay-iban-element">
					<?php esc_html_e( 'IBAN.', 'woocommerce-gateway-youcanpay' ); ?> <span class="required">*</span>
				</label>
				<div id="youcanpay-iban-element" class="wc-youcanpay-iban-element-field">
	...
	public function payment_fields() {
		...
		echo '<div
			id="youcanpay-sepa_debit-payment-data"
			data-amount="' . esc_attr( WC_YouCanPay_Helper::get_youcanpay_amount( $total ) ) . '"
			data-currency="' . esc_attr( strtolower( get_woocommerce_currency() ) ) . '">';

		if ( $this->testmode ) {
			$description .= ' ' . __( 'TEST MODE ENABLED. In test mode, you can use IBAN number DE89370400440532013000.', 'woocommerce-gateway-youcanpay' );
		}
		...
		if ( apply_filters( 'wc_youcanpay_display_save_payment_method_checkbox', $display_tokenization ) && ! is_add_payment_method_page() && ! isset( $_GET['change_payment_method'] ) ) {
			$this->save_payment_method_checkbox();
		}

		do_action( 'wc_youcanpay_payment_fields_youcanpay_sepa', $this->id );
	...
	public function process_payment( $order_id, $retry = true, $force_save_source = false ) {
		try {
			...
			if ( $order->get_total() > 0 ) {
				...
				// Make the request.
				$response = WC_YouCanPay_API::request( $this->generate_payment_request( $order, $prepared_source ) );
				...
				do_action( 'wc_gateway_youcanpay_process_payment', $response, $order );
