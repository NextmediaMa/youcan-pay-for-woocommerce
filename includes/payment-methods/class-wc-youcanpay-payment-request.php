<?php
/**
 * YouCanPay Payment Request API
 * Adds support for Apple Pay and Chrome Payment Request API buttons.
 * Utilizes the YouCanPay Payment Request Button to support checkout from the product detail and cart pages.
 *
 * @package WooCommerce_YouCanPay/Classes/Payment_Request
 * @since   4.0.0
 */
...
/**
 * WC_YouCanPay_Payment_Request class.
 */
class WC_YouCanPay_Payment_Request {
	/**
	 * Enabled.
	 *
	 * @var
	 */
	public $youcanpay_settings;
	...
	public function __construct() {
		self::$_this           = $this;
		$this->youcanpay_settings = get_option( 'woocommerce_youcanpay_settings', [] );
		...
		if ( $this->testmode ) {
			...
			$this->secret_key      = ! empty( $this->youcanpay_settings['test_secret_key'] ) ? $this->youcanpay_settings['test_secret_key'] : '';
		}

		$this->total_label = str_replace( "'", '', $this->total_label ) . apply_filters( 'wc_youcanpay_payment_request_total_label_suffix', ' (via WooCommerce)' );
	...
	public function are_keys_set() {
		...
		// in includes/abstracts/abstract-wc-youcanpay-payment-gateway.php
		if ( $this->testmode ) {
	...
	public function handle_payment_request_redirect() {
		if (
			! empty( $_GET['wc_youcanpay_payment_request_redirect_url'] )
			&& ! empty( $_GET['_wpnonce'] )
			&& wp_verify_nonce( $_GET['_wpnonce'], 'wc-youcanpay-set-redirect-url' ) // @codingStandardsIgnoreLine
		) {
			$url = rawurldecode( esc_url_raw( wp_unslash( $_GET['wc_youcanpay_payment_request_redirect_url'] ) ) );
			...
			wc_setcookie( 'wc_youcanpay_payment_request_redirect_url', $url, time() + MINUTE_IN_SECONDS * 10 );
	...
	public function init() {
		...
		add_action( 'wc_ajax_wc_youcanpay_get_cart_details', [ $this, 'ajax_get_cart_details' ] );
		add_action( 'wc_ajax_wc_youcanpay_get_shipping_options', [ $this, 'ajax_get_shipping_options' ] );
		add_action( 'wc_ajax_wc_youcanpay_update_shipping_method', [ $this, 'ajax_update_shipping_method' ] );
		add_action( 'wc_ajax_wc_youcanpay_create_order', [ $this, 'ajax_create_order' ] );
		add_action( 'wc_ajax_wc_youcanpay_add_to_cart', [ $this, 'ajax_add_to_cart' ] );
		add_action( 'wc_ajax_wc_youcanpay_get_selected_product_data', [ $this, 'ajax_get_selected_product_data' ] );
		add_action( 'wc_ajax_wc_youcanpay_clear_cart', [ $this, 'ajax_clear_cart' ] );
		add_action( 'wc_ajax_wc_youcanpay_log_errors', [ $this, 'ajax_log_errors' ] );
	...
	public function get_button_type() {
		return isset( $this->youcanpay_settings['payment_request_button_type'] ) ? $this->youcanpay_settings['payment_request_button_type'] : 'default';
	}
	...
	public function get_button_theme() {
		return isset( $this->youcanpay_settings['payment_request_button_theme'] ) ? $this->youcanpay_settings['payment_request_button_theme'] : 'dark';
	}
	...
	public function get_button_branded_type() {
		return isset( $this->youcanpay_settings['payment_request_button_branded_type'] ) ? $this->youcanpay_settings['payment_request_button_branded_type'] : 'default';
	}
	...
	public function custom_button_selector() {
		return $this->is_custom_button() ? '#wc-youcanpay-custom-button' : '';
	}
	...
	public function get_product_data() {
		...
		$items[] = [
			...
			'amount' => WC_YouCanPay_Helper::get_youcanpay_amount( $this->get_product_price( $product ) ),
		];

		if ( wc_tax_enabled() ) {
			$items[] = [
				'label'   => __( 'Tax', 'woocommerce-gateway-youcanpay' ),
				'amount'  => 0,
				'pending' => true,
			];
		}
		...
		if ( wc_shipping_enabled() && $product->needs_shipping() ) {
			$items[] = [
				'label'   => __( 'Shipping', 'woocommerce-gateway-youcanpay' ),
				'amount'  => 0,
				'pending' => true,
			];
			...
			$data['shippingOptions'] = [
				'id'     => 'pending',
				'label'  => __( 'Pending', 'woocommerce-gateway-youcanpay' ),
				'detail' => '',
				'amount' => 0,
			];
		}
		...
		$data['total']        = [
			'label'   => apply_filters( 'wc_youcanpay_payment_request_total_label', $this->total_label ),
			'amount'  => WC_YouCanPay_Helper::get_youcanpay_amount( $this->get_product_price( $product ) ),
			'pending' => true,
		];
		...
		return apply_filters( 'wc_youcanpay_payment_request_product_data', $data, $product );
	}
	...
	public function filter_gateway_title( $title, $id ) {
		...
		if ( 'youcanpay' === $id && ! empty( $method_title ) ) {
			if ( 'Apple Pay (YouCanPay)' === $method_title
				|| 'Google Pay (YouCanPay)' === $method_title
				|| 'Payment Request (YouCanPay)' === $method_title
			) {
				return $method_title;
			}
			...
			// Request Buttons.
			if ( 'Chrome Payment Request (YouCanPay)' === $method_title ) {
				return 'Payment Request (YouCanPay)';
			}
	...
	public function add_order_meta( $order_id, $posted_data ) {
		...
		if ( 'apple_pay' === $payment_request_type ) {
			$order->set_payment_method_title( 'Apple Pay (YouCanPay)' );
			$order->save();
		} elseif ( 'google_pay' === $payment_request_type ) {
			$order->set_payment_method_title( 'Google Pay (YouCanPay)' );
			$order->save();
		} elseif ( 'payment_request_api' === $payment_request_type ) {
			$order->set_payment_method_title( 'Payment Request (YouCanPay)' );
			$order->save();
		}
	}
	...
	public function supported_product_types() {
		return apply_filters(
			'wc_youcanpay_payment_request_supported_types',
			[
				'simple',
				'variable',
				'variation',
	...
	public function has_subscription_product() {
		...
		} elseif ( WC_YouCanPay_Helper::has_cart_or_checkout_on_current_page() ) {
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				$_product = apply_filters( 'woocommerce_cart_item_product', $cart_item['data'], $cart_item, $cart_item_key );
	...
	public function get_login_redirect_url( $redirect ) {
		$url = esc_url_raw( wp_unslash( isset( $_COOKIE['wc_youcanpay_payment_request_redirect_url'] ) ? $_COOKIE['wc_youcanpay_payment_request_redirect_url'] : '' ) );

		if ( empty( $url ) ) {
			return $redirect;
		}
		wc_setcookie( 'wc_youcanpay_payment_request_redirect_url', null );
	...
	public function javascript_params() {
		...
		return [
			...
			'youcanpay'             => [
				...
				'allow_prepaid_card' => apply_filters( 'wc_youcanpay_allow_prepaid_card', true ) ? 'yes' : 'no',
			],
			'nonce'              => [
				'payment'                   => wp_create_nonce( 'wc-youcanpay-payment-request' ),
				'shipping'                  => wp_create_nonce( 'wc-youcanpay-payment-request-shipping' ),
				'update_shipping'           => wp_create_nonce( 'wc-youcanpay-update-shipping-method' ),
				'checkout'                  => wp_create_nonce( 'woocommerce-process_checkout' ),
				'add_to_cart'               => wp_create_nonce( 'wc-youcanpay-add-to-cart' ),
				'get_selected_product_data' => wp_create_nonce( 'wc-youcanpay-get-selected-product-data' ),
				'log_errors'                => wp_create_nonce( 'wc-youcanpay-log-errors' ),
				'clear_cart'                => wp_create_nonce( 'wc-youcanpay-clear-cart' ),
			],
			'i18n'               => [
				'no_prepaid_card'  => __( 'Sorry, we\'re not accepting prepaid cards at this time.', 'woocommerce-gateway-youcanpay' ),
				/* translators: Do not translate the [option] placeholder */
				'unknown_shipping' => __( 'Unknown shipping option "[option]".', 'woocommerce-gateway-youcanpay' ),
			],
			'checkout'           => [
	...
	public function scripts() {
		...
		wp_register_script( 'wc_youcanpay_payment_request', plugins_url( 'assets/js/youcanpay-payment-request' . $suffix . '.js', WC_YOUCAN_PAY_MAIN_FILE ), [ 'jquery', 'youcanpay' ], WC_YOUCAN_PAY_VERSION, true );

		wp_localize_script(
			'wc_youcanpay_payment_request',
			'wc_youcanpay_payment_request_params',
			apply_filters(
				'wc_youcanpay_payment_request_params',
		...
		wp_enqueue_script( 'wc_youcanpay_payment_request' );

		$gateways = WC()->payment_gateways->get_available_payment_gateways();
		if ( isset( $gateways['youcanpay'] ) ) {
			$gateways['youcanpay']->payment_scripts();
		}
	}
	...
	public function display_payment_request_button_html() {
		...
		?>
		<div id="wc-youcanpay-payment-request-wrapper" style="clear:both;padding-top:1.5em;display:none;">
			<div id="wc-youcanpay-payment-request-button">
				<?php
				if ( $this->is_custom_button() ) {
					...
					echo "<button id=\"wc-youcanpay-custom-button\" class=\"$class_name\" style=\"$style\"> $label </button>";
				}
				?>
	...
	public function display_payment_request_button_separator_html() {
		...
		?>
		<p id="wc-youcanpay-payment-request-button-separator" style="margin-top:1.5em;text-align:center;display:none;">&mdash; <?php esc_html_e( 'OR', 'woocommerce-gateway-youcanpay' ); ?> &mdash;</p>
		<?php
	}
	...
	public function should_show_payment_request_button() {
		...
		if ( ! $this->testmode && ! is_ssl() ) {
			WC_YouCanPay_Logger::log( 'YouCanPay Payment Request live mode requires SSL.' );
			return false;
		}
	...
	public function should_show_prb_on_cart_page() {
		...
		$deprecation_message      =
			'Please configure Payment Request Button locations through the YouCanPay plugin settings.';
		...
		return apply_filters_deprecated(
			'wc_youcanpay_show_payment_request_on_cart',
	...
	public function should_show_prb_on_checkout_page() {
		...
		// Respect the deprecated filters, but add a deprecation notice.
		return apply_filters_deprecated(
			'wc_youcanpay_show_payment_request_on_checkout',
	...
	public function should_show_prb_on_product_pages() {
		...
		return ! apply_filters_deprecated(
			'wc_youcanpay_hide_payment_request_on_product_page',
	...
	public function ajax_log_errors() {
		check_ajax_referer( 'wc-youcanpay-log-errors', 'security' );

		$errors = isset( $_POST['errors'] ) ? wc_clean( wp_unslash( $_POST['errors'] ) ) : '';

		WC_YouCanPay_Logger::log( $errors );
	...
	public function ajax_clear_cart() {
		check_ajax_referer( 'wc-youcanpay-clear-cart', 'security' );
	...
	public function ajax_get_cart_details() {
		check_ajax_referer( 'wc-youcanpay-payment-request', 'security' );
	...
	public function ajax_get_shipping_options() {
		check_ajax_referer( 'wc-youcanpay-payment-request-shipping', 'security' );
	...
	public function get_shipping_options( $shipping_address, $itemized_display_items = false ) {
		try {
			...
			$this->calculate_shipping( apply_filters( 'wc_youcanpay_payment_request_shipping_posted_values', $shipping_address ) );
			...
			if ( ! empty( $packages ) && WC()->customer->has_calculated_shipping() ) {
				foreach ( $packages as $package_key => $package ) {
					if ( empty( $package['rates'] ) ) {
						throw new Exception( __( 'Unable to find shipping method for address.', 'woocommerce-gateway-youcanpay' ) );
					}
					...
					foreach ( $package['rates'] as $key => $rate ) {
						if ( in_array( $rate->id, $shipping_rate_ids, true ) ) {
							...
							// option IDs.
							throw new Exception( __( 'Unable to provide shipping options for Payment Requests.', 'woocommerce-gateway-youcanpay' ) );
						}
						...
						$data['shipping_options'][] = [
							...
							'label'  => $rate->label,
							'detail' => '',
							'amount' => WC_YouCanPay_Helper::get_youcanpay_amount( $rate->cost ),
						];
					}
				}
			} else {
	...
	public function ajax_update_shipping_method() {
		check_ajax_referer( 'wc-youcanpay-update-shipping-method', 'security' );
	...
	public function ajax_get_selected_product_data() {
		check_ajax_referer( 'wc-youcanpay-get-selected-product-data', 'security' );
		...
		try {
			...
			if ( ! is_a( $product, 'WC_Product' ) ) {
				...
				throw new Exception( sprintf( __( 'Product with the ID (%d) cannot be found.', 'woocommerce-gateway-youcanpay' ), $product_id ) );
			}
			...
			if ( $product->is_sold_individually() ) {
				$qty = apply_filters( 'wc_youcanpay_payment_request_add_to_cart_sold_individually_quantity', 1, $qty, $product_id, $variation_id );
			}

			if ( ! $product->has_enough_stock( $qty ) ) {
				/* translators: 1: product name 2: quantity in stock */
				throw new Exception( sprintf( __( 'You cannot add that amount of "%1$s"; to the cart because there is not enough stock (%2$s remaining).', 'woocommerce-gateway-youcanpay' ), $product->get_name(), wc_format_stock_quantity_for_display( $product->get_stock_quantity(), $product ) ) );
			}
			...
			$items[] = [
				...
				'amount' => WC_YouCanPay_Helper::get_youcanpay_amount( $total ),
			];
	...
	public function ajax_add_to_cart() {
		check_ajax_referer( 'wc-youcanpay-add-to-cart', 'security' );
	...
	public function get_normalized_state_from_pr_states( $state, $country ) {
		...
		include_once WC_YOUCAN_PAY_PLUGIN_PATH . '/includes/constants/class-wc-youcanpay-payment-request-button-states.php';
		$pr_states = WC_YouCanPay_Payment_Request_Button_States::STATES;
		...
		if ( ! $is_supported ) {
			wc_add_notice(
				sprintf(
					...
					__( 'The Payment Request button is not supported in %s because some required fields couldn\'t be verified. Please proceed to the checkout page and try again.', 'woocommerce-gateway-youcanpay' ),
	...
	public function ajax_create_order() {
		if ( WC()->cart->is_empty() ) {
			wp_send_json_error( __( 'Empty cart', 'woocommerce-gateway-youcanpay' ) );
		}
	...
	protected function build_shipping_methods( $shipping_methods ) {
		...
		foreach ( $shipping_methods as $method ) {
			$shipping[] = [
				...
				'label'  => $method['label'],
				'detail' => '',
				'amount' => WC_YouCanPay_Helper::get_youcanpay_amount( $method['amount']['value'] ),
			];
		}
	...
	protected function build_display_items( $itemized_display_items = false ) {
		...
		$subtotal      = 0;
		$discounts     = 0;
		$display_items = ! apply_filters( 'wc_youcanpay_payment_request_hide_itemization', true ) || $itemized_display_items;
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			$lines[] = [
				...
				'amount' => WC_YouCanPay_Helper::get_youcanpay_amount( $amount ),
			];
		}
		...
		if ( $display_items ) {
			...
			$items[] = [
				'label'  => 'Subtotal',
				'amount' => WC_YouCanPay_Helper::get_youcanpay_amount( $subtotal ),
			];
		}
		...
		if ( wc_tax_enabled() ) {
			$items[] = [
				'label'  => esc_html( __( 'Tax', 'woocommerce-gateway-youcanpay' ) ),
				'amount' => WC_YouCanPay_Helper::get_youcanpay_amount( $tax ),
			];
		}

		if ( WC()->cart->needs_shipping() ) {
			$items[] = [
				'label'  => esc_html( __( 'Shipping', 'woocommerce-gateway-youcanpay' ) ),
				'amount' => WC_YouCanPay_Helper::get_youcanpay_amount( $shipping ),
			];
		}

		if ( WC()->cart->has_discount() ) {
			$items[] = [
				'label'  => esc_html( __( 'Discount', 'woocommerce-gateway-youcanpay' ) ),
				'amount' => WC_YouCanPay_Helper::get_youcanpay_amount( $discounts ),
			];
		}
		...
		foreach ( $cart_fees as $key => $fee ) {
			$items[] = [
				'label'  => $fee->name,
				'amount' => WC_YouCanPay_Helper::get_youcanpay_amount( $fee->amount ),
			];
		}
		...
		return [
			...
			'total'        => [
				'label'   => $this->total_label,
				'amount'  => max( 0, apply_filters( 'woocommerce_youcanpay_calculated_total', WC_YouCanPay_Helper::get_youcanpay_amount( $order_total ), $order_total, WC()->cart ) ),
				'pending' => false,
			],
		];
	}
	...
	public function get_login_confirmation_settings() {
		...
		$message      = __( 'To complete your transaction with **the selected payment method**, you must log in or create an account with our site.', 'woocommerce-gateway-youcanpay' );
		$redirect_url = add_query_arg(
			[
				'_wpnonce'                               => wp_create_nonce( 'wc-youcanpay-set-redirect-url' ),
				'wc_youcanpay_payment_request_redirect_url' => rawurlencode( home_url( add_query_arg( [] ) ) ), // Current URL to redirect to after login.
			],
			home_url()
		);
	...
	public function get_button_locations() {
		...
		if ( ! isset( $this->youcanpay_settings['payment_request_button_locations'] ) ) {
			return [ 'product', 'cart' ];
		}
		...
		if ( ! is_array( $this->youcanpay_settings['payment_request_button_locations'] ) ) {
			return [];
		}

		return $this->youcanpay_settings['payment_request_button_locations'];
	}
}
