...
class WC_YouCanPay_Customer {
	...
	/**
	 * String prefix for YouCan Pay payment methods request transient.
	 */
	const PAYMENT_METHODS_TRANSIENT_KEY = 'youcanpay_payment_methods_';
	...
	/**
	 * Store data from the YouCan Pay API about this customer
	 */
	...
	protected function generate_customer_request( $args = [] ) {
		...
		$metadata                      = [];
		$defaults['metadata']          = apply_filters( 'wc_youcanpay_customer_metadata', $metadata, $user );
	...
	public function create_customer( $args = [] ) {
		...
		$response = WC_YouCanPay_API::request( apply_filters( 'wc_youcanpay_create_customer_args', $args ), 'customers' );
		...
		do_action( 'woocommerce_youcanpay_add_customer', $args, $response );

		return $response->id;
	}

	/**
	 * Updates the YouCan Pay customer through the API.
	 *
	...
	public function update_customer( $args = [], $is_retry = false ) {
		...
		$args     = $this->generate_customer_request( $args );
		$args     = apply_filters( 'wc_youcanpay_update_customer_args', $args );
		$response = WC_YouCanPay_API::request( $args, 'customers/' . $this->get_id() );

		if ( ! empty( $response->error ) ) {
			if ( $this->is_no_such_customer_error( $response->error ) && ! $is_retry ) {
				// This can happen when switching the main YouCan Pay account or importing users from another site.
		...
		do_action( 'woocommerce_youcanpay_update_customer', $args, $response );

		return $this->get_id();
	}

	/**
	 * Updates existing YouCan Pay customer or creates new customer for User through API.
	 *
	 * @param array $args     Additional arguments for the request (optional).
	 * @param bool  $is_retry Whether the current call is a retry (optional, defaults to false). If true, then an exception will be thrown instead of further retries on error.
	 *
	...
	/**
	 * Add a source for this youcanpay customer.
	 *
	 * @param string $source_id
	 * @return WP_Error|int
	 */
	public function add_source( $source_id ) {
		$response = WC_YouCanPay_API::retrieve( 'sources/' . $source_id );
			...
			if ( ! empty( $response->type ) ) {
				switch ( $response->type ) {
					case 'standalone':
						break;
					case 'sepa_debit':
						$wc_token = new WC_YouCan_Pay_Token_SEPA();
						$wc_token->set_token( $response->id );
						$wc_token->set_gateway_id( 'youcanpay_sepa' );
					...
					default:
						if ( 'source' === $response->object && 'card' === $response->type ) {
							...
							$wc_token->set_token( $response->id );
							$wc_token->set_gateway_id( 'youcanpay' );
		...
		do_action( 'woocommerce_youcanpay_add_source', $this->get_id(), $wc_token, $response, $source_id );
	...
	public function attach_source( $source_id ) {
		...
		$response = WC_YouCanPay_API::request(
			[
				'source' => $source_id,
			],
		...
		if ( ! empty( $response->error ) ) {
			...
			if ( $this->is_no_such_customer_error( $response->error ) ) {
				...
				return WC_YouCanPay_API::request( [], 'sources/' . $source_id, 'GET' );
			} else {
				return $response;
			}
	...
	public function get_sources() {
		...
		$sources = get_transient( 'youcanpay_sources_' . $this->get_id() );
		...
		if ( false === $sources ) {
			...
			set_transient( 'youcanpay_sources_' . $this->get_id(), $sources, DAY_IN_SECONDS );
		}
	...
	public function get_payment_methods( $payment_method_type ) {
		...
		if ( false === $payment_methods ) {
			$params   = WC_YouCanPay_UPE_Payment_Method_Sepa::YOUCAN_PAY_ID === $payment_method_type ? '?expand[]=data.sepa_debit.generated_from.charge&expand[]=data.sepa_debit.generated_from.setup_attempt' : '';
			$response = WC_YouCanPay_API::request(
				[
	...
	/**
	 * Delete a source from youcanpay.
	 *
	 * @param string $source_id
	 */
	public function delete_source( $source_id ) {
		...
		$response = WC_YouCanPay_API::request( [], 'customers/' . $this->get_id() . '/sources/' . sanitize_text_field( $source_id ), 'DELETE' );

		$this->clear_cache();

		if ( empty( $response->error ) ) {
			do_action( 'wc_youcanpay_delete_source', $this->get_id(), $response );
	...
	/**
	 * Detach a payment method from youcanpay.
	 *
	...
	public function detach_payment_method( $payment_method_id ) {
		...
		$response = WC_YouCanPay_API::request( [], "payment_methods/$payment_method_id/detach", 'POST' );

		$this->clear_cache();

		if ( empty( $response->error ) ) {
			do_action( 'wc_youcanpay_detach_payment_method', $this->get_id(), $response );
	...
	public function set_default_source( $source_id ) {
		...
		if ( empty( $response->error ) ) {
			do_action( 'wc_youcanpay_set_default_source', $this->get_id(), $response );
	...
	public function set_default_payment_method( $payment_method_id ) {
		...
		if ( empty( $response->error ) ) {
			do_action( 'wc_youcanpay_set_default_payment_method', $this->get_id(), $response );
	...
	public function clear_cache() {
		delete_transient( 'youcanpay_sources_' . $this->get_id() );
		delete_transient( 'youcanpay_customer_' . $this->get_id() );
		foreach ( self::YOUCAN_PAY_PAYMENT_METHODS as $payment_method_type ) {
	...
	public function delete_id_from_meta() {
		delete_user_option( $this->get_user_id(), '_youcanpay_customer_id', false );
	}
	...
	public function get_customer_preferred_locale( $user ) {
		...
		$preferred = isset( $youcanpay_locales[ $locale ] ) ? $youcanpay_locales[ $locale ] : 'en-US';
		return [ $preferred ];
	}
