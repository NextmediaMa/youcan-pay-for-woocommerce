<?php
/**
 * Class Allowed_YouCan_Pay_Request_Button_Types_Update
 */
...
class Allowed_YouCan_Pay_Request_Button_Types_Update {
	/**
	 * Allowed_YouCan_Pay_Request_Button_Types_Update constructor.
	 */
	public function __construct() {
		add_action( 'woocommerce_youcanpay_updated', [ $this, 'maybe_migrate' ] );
	}
	...
	public function maybe_migrate() {
		...
		$youcanpay_gateway = $this->get_gateway();

		// "custom" or "branded" are no longer valid values for the button type - map them to new ones
		$button_type = $youcanpay_gateway->get_option( 'payment_request_button_type' );
		if ( in_array( $button_type, [ 'branded', 'custom' ], true ) ) {
			$branded_type = $youcanpay_gateway->get_option( 'payment_request_button_branded_type' );
			$youcanpay_gateway->update_option(
	...
	/**
	 * Returns the main YouCanPay payment gateways.
	 *
	 * @return WC_YouCanPay_Payment_Gateway
	 */
	public function get_gateway() {
		return woocommerce_gateway_youcanpay()->get_main_youcanpay_gateway();
	}
}
