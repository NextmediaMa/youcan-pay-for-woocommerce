...
abstract class WC_YouCanPay_Payment_Gateway extends WC_Payment_Gateway_CC {
	...
	public function display_admin_settings_webhook_description() {
		...
		$description = sprintf( __( 'You must add the following webhook endpoint <strong style="background-color:#ddd;">&nbsp;%s&nbsp;</strong> to your <a href="https://dashboard.youcanpay.com/account/webhooks" target="_blank">YouCanPay account settings</a> (if there isn\'t one already enabled). This will enable you to receive notifications on the charge statuses.', 'woocommerce-gateway-youcanpay' ), WC_YouCanPay_Helper::get_webhook_url() );
	...
	/**
	 * Inserts an empty placeholder div for new account card when YouCanPay is not connected.
	...
	public function render_upe_settings() {
		...
		if ( ! $is_youcanpay_connected ) {
			$hide_save_button = true;
			echo '<div id="wc-youcanpay-new-account-container"></div>';
		} else {
			echo '<table class="form-table">' . $this->generate_settings_html( array_slice( $form_fields, 0, $target_index, true ), false ) . '</table>';
			echo '<div id="wc-youcanpay-upe-opt-in-banner"></div>';
	...
	public function admin_scripts_for_banner() {
		...
		if ( ! WC_YouCanPay_Helper::should_enqueue_in_current_tab_section( 'checkout', $this->id ) ) {
			return;
		}

		// Webpack generates an assets file containing a dependencies array for our built JS file.
		$script_asset_path = WC_YOUCAN_PAY_PLUGIN_PATH . '/build/upe_opt_in_banner.asset.php';
		...
		wp_register_script(
			'woocommerce_youcanpay_upe_opt_in',
			plugins_url( 'build/upe_opt_in_banner.js', WC_YOUCAN_PAY_MAIN_FILE ),
		...
		wp_localize_script(
			'woocommerce_youcanpay_upe_opt_in',
			'wc_youcanpay_upe_opt_in_params',
			[
				'method_name' => $this->get_method_title(),
			]
		);
		wp_enqueue_script( 'woocommerce_youcanpay_upe_opt_in' );
	}
	...
	public function save_payment_method_requested() {
		$payment_method = isset( $_POST['payment_method'] ) ? wc_clean( wp_unslash( $_POST['payment_method'] ) ) : 'youcanpay';
	 ...
	 *
	 * @since 4.2.0
	 * @param object   $error The error that was returned from YouCanPay's API.
	 ...
	 * @version 4.0.0
	 */
	public function get_youcanpay_customer_id( $order ) {
		// Try to get it via the order first.
		$customer = $order->get_meta( '_youcanpay_customer_id', true );

		if ( empty( $customer ) ) {
			$customer = get_user_option( '_youcanpay_customer_id', $order->get_customer_id() );
		}
	 ...
	 * @param  object   $prepared_payment_method YouCanPay Payment Method or Source.
	 * @return array()
	 */
	public function generate_payment_request( $order, $prepared_payment_method ) {
		...
		$post_data['metadata'] = apply_filters( 'wc_youcanpay_payment_metadata', $metadata, $order, $prepared_payment_method );
		 ...
		 * @param object $source
		 */
		return apply_filters( 'wc_youcanpay_generate_payment_request', $post_data, $order, $prepared_payment_method );
	}

	/**
	 * Store extra meta data for an order from a YouCanPay Response.
	 */
	public function process_response( $response, $order ) {
		WC_YouCanPay_Logger::log( 'Processing response: ' . print_r( $response, true ) );

		$order_id = $order->get_id();
		$captured = ( isset( $response->captured ) && $response->captured ) ? 'yes' : 'no';

		// Store charge data.
		$order->update_meta_data( '_youcanpay_charge_captured', $captured );
		...
		do_action( 'wc_gateway_youcanpay_process_response', $response, $order );
	...
	public function get_source_object( $source_id = '' ) {
		...
		$source_object = WC_YouCanPay_API::retrieve( 'sources/' . $source_id );

		if ( ! empty( $source_object->error ) ) {
			throw new WC_YouCanPay_Exception( print_r( $source_object, true ), $source_object->error->message );
		}
	 ...
	 * @throws WC_YouCanPay_Exception An exception if the card is prepaid, but prepaid cards are not allowed.
	 */
	public function maybe_disallow_prepaid_card( $payment_method ) {
		// Check if we don't allow prepaid credit cards.
		if ( apply_filters( 'wc_youcanpay_allow_prepaid_card', true ) || ! $this->is_prepaid_card( $payment_method ) ) {
			return;
		}
	 ...
	 * If user is logged in and/or has WC account, create an account on YouCanPay.
	...
	public function prepare_source( $user_id, $force_save_source = false, $existing_customer_id = null ) {
		...
		$force_save_source = apply_filters( 'wc_youcanpay_force_save_source', $force_save_source, $customer );
		$source_object     = '';
		$source_id         = '';
		$wc_token_id       = false;
		$payment_method    = isset( $_POST['payment_method'] ) ? wc_clean( wp_unslash( $_POST['payment_method'] ) ) : 'youcanpay';
		...
		if ( ! empty( $_POST['youcanpay_source'] ) ) {
			$source_object = self::get_source_object( wc_clean( wp_unslash( $_POST['youcanpay_source'] ) ) );
		...
		} elseif ( isset( $_POST['youcanpay_token'] ) && 'new' !== $_POST['youcanpay_token'] ) {
			$youcanpay_token     = wc_clean( wp_unslash( $_POST['youcanpay_token'] ) );
			$maybe_saved_card = isset( $_POST[ 'wc-' . $payment_method . '-new-payment-method' ] ) && ! empty( $_POST[ 'wc-' . $payment_method . '-new-payment-method' ] );
	...
	public function prepare_order_source( $order = null ) {
		...
		if ( $order ) {
			...
			$youcanpay_customer_id = $this->get_youcanpay_customer_id( $order );

			if ( $youcanpay_customer_id ) {
				$youcanpay_customer->set_id( $youcanpay_customer_id );
			}

			$source_id = $order->get_meta( '_youcanpay_source_id', true );

			// Since 4.0.0, we changed card to source so we need to account for that.
			if ( empty( $source_id ) ) {
				$source_id = $order->get_meta( '_youcanpay_card_id', true );

				// Take this opportunity to update the key name.
				$order->update_meta_data( '_youcanpay_source_id', $source_id );
			...
			if ( $source_id ) {
				$youcanpay_source = $source_id;
				$source_object = WC_YouCanPay_API::retrieve( 'sources/' . $source_id );
			} elseif ( apply_filters( 'wc_youcanpay_use_default_customer_source', true ) ) {
				/*
		...
		return (object) [
			'token_id'       => $token_id,
			'customer'       => $youcanpay_customer ? $youcanpay_customer->get_id() : false,
			'source'         => $youcanpay_source,
			'source_object'  => $source_object,
			'payment_method' => null,
		];
	}
	 ...
	 * @throws WC_YouCanPay_Exception     An exception if the source ID is missing.
	 */
	...
	public function save_source_to_order( $order, $source ) {
		...
		if ( $source->customer ) {
			$order->update_meta_data( '_youcanpay_customer_id', $source->customer );
		}

		if ( $source->source ) {
			$order->update_meta_data( '_youcanpay_source_id', $source->source );
		}
	...
	public function update_fees( $order, $balance_transaction_id ) {
		$balance_transaction = WC_YouCanPay_API::retrieve( 'balance/history/' . $balance_transaction_id );

		if ( empty( $balance_transaction->error ) ) {
			if ( isset( $balance_transaction ) && isset( $balance_transaction->fee ) ) {
				// Fees and Net needs to both come from YouCanPay to be accurate as the returned
				// values are in the local currency of the YouCanPay account, not from WC.
				$fee_refund = ! empty( $balance_transaction->fee ) ? WC_YouCanPay_Helper::format_balance_fee( $balance_transaction, 'fee' ) : 0;
				$net_refund = ! empty( $balance_transaction->net ) ? WC_YouCanPay_Helper::format_balance_fee( $balance_transaction, 'net' ) : 0;

				// Current data fee & net.
				$fee_current = WC_YouCanPay_Helper::get_youcanpay_fee( $order );
				$net_current = WC_YouCanPay_Helper::get_youcanpay_net( $order );

				// Calculation.
				$fee = (float) $fee_current + (float) $fee_refund;
				$net = (float) $net_current + (float) $net_refund;

				WC_YouCanPay_Helper::update_youcanpay_fee( $order, $fee );
				WC_YouCanPay_Helper::update_youcanpay_net( $order, $net );

				$currency = ! empty( $balance_transaction->currency ) ? strtoupper( $balance_transaction->currency ) : null;
				WC_YouCanPay_Helper::update_youcanpay_currency( $order, $currency );
			...
			WC_YouCanPay_Logger::log( 'Unable to update fees/net meta for order: ' . $order->get_id() );
		}
	}
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( ! is_null( $amount ) ) {
			$request['amount'] = WC_YouCanPay_Helper::get_youcanpay_amount( $amount, $order_currency );
		}
		...
		if ( $reason ) {
			// Trim the refund reason to a max of 500 characters due to YouCanPay limits: https://youcanpay.com/docs/api/metadata.
		...
		WC_YouCanPay_Logger::log( "Info: Beginning refund for order {$charge_id} for the amount of {$amount}" );

		$request = apply_filters( 'wc_youcanpay_refund_request', $request, $order );
		...
		if ( $intent ) {
			...
			if ( ! empty( $intent->error ) ) {
				...
				$result           = WC_YouCanPay_API::request(
					[],
		...
		if ( ! $intent_cancelled && 'yes' === $captured ) {
			$response = WC_YouCanPay_API::request( $request, 'refunds' );
		}

		if ( ! empty( $response->error ) ) {
			WC_YouCanPay_Logger::log( 'Error: ' . $response->error->message );
			...
			if ( in_array( strtolower( $order->get_currency() ), WC_YouCanPay_Helper::no_decimal_currencies(), true ) ) {
			...
			$order->update_meta_data( '_youcanpay_refund_id', $response->id );
	...
	public function generate_create_intent_request( $order, $prepared_source ) {
		...
		$force_save_source = apply_filters( 'wc_youcanpay_force_save_source', false, $prepared_source->source );
		 ...
		 * @param object $source
		 */
		return apply_filters( 'wc_youcanpay_generate_create_intent_request', $request, $order, $prepared_source );
	}

	/**
	 * Create the level 3 data array to send to YouCanPay when making a purchase.
	 *
	 * @param WC_Order $order The order that is being paid for.
	 * @return array          The level 3 data to send to YouCanPay.
	 */
	public function get_level3_data_from_order( $order ) {
		...
		$youcanpay_line_items = array_map(
			function( $item ) use ( $currency ) {
				...
				$unit_cost           = WC_YouCanPay_Helper::get_youcanpay_amount( ( $subtotal / $quantity ), $currency );
				$tax_amount          = WC_YouCanPay_Helper::get_youcanpay_amount( $item->get_total_tax(), $currency );
				$discount_amount     = WC_YouCanPay_Helper::get_youcanpay_amount( $subtotal - $item->get_total(), $currency );
		...
		$level3_data = [
			...
			'shipping_amount'    => WC_YouCanPay_Helper::get_youcanpay_amount( (float) $order->get_shipping_total() + (float) $order->get_shipping_tax(), $currency ), // The shipping cost, in cents, as a non-negative integer.
			'line_items'         => $youcanpay_line_items,
		];
	...
	public function create_intent( $order, $prepared_source ) {
		...
		$intent = WC_YouCanPay_API::request( $request, 'payment_intents' );
		if ( ! empty( $intent->error ) ) {
			return $intent;
		}

		$order_id = $order->get_id();
		WC_YouCanPay_Logger::log( "YouCanPay PaymentIntent $intent->id initiated for order $order_id" );
	...
	public function update_existing_intent( $intent, $order, $prepared_source ) {
		...
		$new_amount = WC_YouCanPay_Helper::get_youcanpay_amount( $order->get_total() );
		...
		return WC_YouCanPay_API::request_with_level3_data(
			$request,
	...
	public function confirm_intent( $intent, $order, $prepared_source ) {
		...
		$level3_data      = $this->get_level3_data_from_order( $order );
		$confirmed_intent = WC_YouCanPay_API::request_with_level3_data(
			$confirm_request,
		...
		if ( 'succeeded' === $confirmed_intent->status ) {
			...
			WC_YouCanPay_Logger::log( "YouCanPay PaymentIntent $intent->id requires authentication for order $order_id" );
		}
	...
	public function save_intent_to_order( $order, $intent ) {
		if ( 'payment_intent' === $intent->object ) {
			$order->update_meta_data( '_youcanpay_intent_id', $intent->id );
		} elseif ( 'setup_intent' === $intent->object ) {
			$order->update_meta_data( '_youcanpay_setup_intent', $intent->id );
		}
	...
	public function get_intent_from_order( $order ) {
		$intent_id = $order->get_meta( '_youcanpay_intent_id' );
		...
		$intent_id = $order->get_meta( '_youcanpay_setup_intent' );
	...
	/**
	 * Retrieves intent from YouCanPay API by intent id.
	 *
	...
	private function get_intent( $intent_type, $intent_id ) {
		...
		$response = WC_YouCanPay_API::request( [], "$intent_type/$intent_id?expand[]=payment_method", 'GET' );

		if ( $response && isset( $response->{ 'error' } ) ) {
			$error_response_message = print_r( $response, true );
			WC_YouCanPay_Logger::log( "Failed to get YouCanPay intent $intent_type/$intent_id." );
			WC_YouCanPay_Logger::log( "Response: $error_response_message" );
			return false;
		}
	...
	public function lock_order_payment( $order, $intent = null ) {
		$order_id       = $order->get_id();
		$transient_name = 'wc_youcanpay_processing_intent_' . $order_id;
