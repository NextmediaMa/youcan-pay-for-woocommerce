...
/**
 * WC_YouCanPay_Intent_Controller class.
 *
 * Handles in-checkout AJAX calls, related to Payment Intents.
 */
class WC_YouCanPay_Intent_Controller {
	/**
	 * Holds an instance of the gateway class.
	 *
	 * @since 4.2.0
	 * @var WC_Gateway_YouCanPay
	 */
	protected $gateway;
	...
	public function __construct() {
		add_action( 'wc_ajax_wc_youcanpay_verify_intent', [ $this, 'verify_intent' ] );
		add_action( 'wc_ajax_wc_youcanpay_create_setup_intent', [ $this, 'create_setup_intent' ] );

		add_action( 'wc_ajax_wc_youcanpay_create_payment_intent', [ $this, 'create_payment_intent_ajax' ] );
		add_action( 'wc_ajax_wc_youcanpay_update_payment_intent', [ $this, 'update_payment_intent_ajax' ] );
		add_action( 'wc_ajax_wc_youcanpay_init_setup_intent', [ $this, 'init_setup_intent_ajax' ] );

		add_action( 'wc_ajax_wc_youcanpay_save_upe_appearance', [ $this, 'save_upe_appearance_ajax' ] );
		add_action( 'wc_ajax_nopriv_wc_youcanpay_save_upe_appearance', [ $this, 'save_upe_appearance_ajax' ] );

		add_action( 'wc_ajax_wc_youcanpay_update_order_status', [ $this, 'update_order_status_ajax' ] );
		add_action( 'wc_ajax_wc_youcanpay_update_failed_order', [ $this, 'update_failed_order_ajax' ] );
	...
	protected function get_gateway() {
		if ( ! isset( $this->gateway ) ) {
			...
			$this->gateway = $gateways[ WC_Gateway_YouCanPay::ID ];
		}
	 ...
	 *
	 * @since 5.6.0
	 * @throws WC_YouCanPay_Exception if UPE is not enabled.
	 * @return WC_YouCanPay_UPE_Payment_Gateway
	 */
	protected function get_upe_gateway() {
		$gateway = $this->get_gateway();
		if ( ! $gateway instanceof WC_YouCanPay_UPE_Payment_Gateway ) {
			WC_YouCanPay_Logger::log( 'Error instantiating the UPE Payment Gateway, UPE is not enabled.' );
			throw new WC_YouCanPay_Exception( __( "We're not able to process this payment.", 'woocommerce-gateway-youcanpay' ) );
		}
		return $gateway;
	}
	 ...
	 *
	 * @since 4.2.0
	 * @throws WC_YouCanPay_Exception An exception if there is no order ID or the order does not exist.
	 * @return WC_Order
	 */
	protected function get_order_from_request() {
		if ( ! isset( $_GET['nonce'] ) || ! wp_verify_nonce( sanitize_key( $_GET['nonce'] ), 'wc_youcanpay_confirm_pi' ) ) {
			throw new WC_YouCanPay_Exception( 'missing-nonce', __( 'CSRF verification failed.', 'woocommerce-gateway-youcanpay' ) );
		}
		...
		if ( ! $order ) {
			throw new WC_YouCanPay_Exception( 'missing-order', __( 'Missing order ID for payment confirmation', 'woocommerce-gateway-youcanpay' ) );
		}
	...
	public function verify_intent() {
		...
		try {
			...
			$message = sprintf( __( 'Payment verification error: %s', 'woocommerce-gateway-youcanpay' ), $e->getLocalizedMessage() );
		...
		try {
			...
			if ( isset( $_GET['save_payment_method'] ) && ! empty( $_GET['save_payment_method'] ) ) {
				...
				if ( isset( $intent->last_payment_error ) ) {
					// Currently, YouCanPay saves the payment method even if the authentication fails for 3DS cards.
					// Although, the card is not stored in DB we need to remove the source from the customer on YouCanPay
					...
					if ( isset( $metadata->save_payment_method ) && 'true' === $metadata->save_payment_method ) {
						$source_object = WC_YouCanPay_API::retrieve( 'sources/' . $intent->source );
	 ...
	 *
	 * @since 4.2.0
	 * @param WC_YouCanPay_Exception $e           The exception that was thrown.
	...
	protected function handle_error( $e, $redirect_url ) {
		...
		WC_YouCanPay_Logger::log( $message );
	...
	public function create_setup_intent() {
		if (
			! is_user_logged_in()
			|| ! isset( $_POST['youcanpay_source_id'] )
			|| ! isset( $_POST['nonce'] )
		) {
			return;
		}
		...
		try {
			$source_id = wc_clean( wp_unslash( $_POST['youcanpay_source_id'] ) );

			// 1. Verify.
			if (
				! wp_verify_nonce( sanitize_key( $_POST['nonce'] ), 'wc_youcanpay_create_si' )
				|| ! preg_match( '/^src_.*$/', $source_id )
			) {
				throw new Exception( __( 'Unable to verify your request. Please reload the page and try again.', 'woocommerce-gateway-youcanpay' ) );
			}
			...
			if ( ! empty( $setup_intent->error ) ) {
				...
				WC_YouCanPay_Logger::log( 'Failed create Setup Intent while saving a card.' );
				WC_YouCanPay_Logger::log( "Response: $error_response_message" );
				throw new Exception( __( 'Your card could not be set up for future usage.', 'woocommerce-gateway-youcanpay' ) );
			}
			...
			if ( 'requires_action' === $setup_intent->status ) {
				...
				$response = [
					...
					'error'  => [
						...
						'message' => __( 'Failed to save payment method.', 'woocommerce-gateway-youcanpay' ),
					],
				];
			} else {
	...
	/**
	 * Handle AJAX requests for creating a payment intent for YouCanPay UPE.
	 */
	public function create_payment_intent_ajax() {
		try {
			$is_nonce_valid = check_ajax_referer( 'wc_youcanpay_create_payment_intent_nonce', false, false );
			if ( ! $is_nonce_valid ) {
				throw new Exception( __( "We're not able to process this payment. Please refresh the page and try again.", 'woocommerce-gateway-youcanpay' ) );
			}

			// If paying from order, we need to get the total from the order instead of the cart.
			$order_id = isset( $_POST['youcanpay_order_id'] ) ? absint( $_POST['youcanpay_order_id'] ) : null;

			wp_send_json_success( $this->create_payment_intent( $order_id ), 200 );
		} catch ( Exception $e ) {
			WC_YouCanPay_Logger::log( 'Create payment intent error: ' . $e->getMessage() );
	...
	public function create_payment_intent( $order_id = null ) {
		...
		$payment_intent = WC_YouCanPay_API::request(
			[
				'amount'               => WC_YouCanPay_Helper::get_youcanpay_amount( $amount, strtolower( $currency ) ),
	...
	/**
	 * Handle AJAX request for updating a payment intent for YouCanPay UPE.
	 *
	 * @since 5.6.0
	 */
	public function update_payment_intent_ajax() {
		try {
			$is_nonce_valid = check_ajax_referer( 'wc_youcanpay_update_payment_intent_nonce', false, false );
			if ( ! $is_nonce_valid ) {
				throw new Exception( __( "We're not able to process this payment. Please refresh the page and try again.", 'woocommerce-gateway-youcanpay' ) );
			}
	...
	public function update_payment_intent( $payment_intent_id = '', $order_id = null, $save_payment_method = false, $selected_upe_payment_type = '' ) {
		...
		if ( $payment_intent_id ) {
			...
			WC_YouCanPay_API::request_with_level3_data(
				$request,
	...
	/**
	 * Handle AJAX requests for creating a setup intent without confirmation for YouCanPay UPE.
	 *
	 * @since 5.6.0
	 * @version 5.6.0
	 */
	public function init_setup_intent_ajax() {
		try {
			$is_nonce_valid = check_ajax_referer( 'wc_youcanpay_create_setup_intent_nonce', false, false );
			if ( ! $is_nonce_valid ) {
				throw new Exception( __( "We're not able to add this payment method. Please refresh the page and try again.", 'woocommerce-gateway-youcanpay' ) );
			}
	...
	public function init_setup_intent() {
		...
		if ( empty( $customer_id ) ) {
			$customer_data = WC_YouCanPay_Customer::map_customer_data( null, new WC_Customer( $user->ID ) );
	...
	public function save_upe_appearance_ajax() {
		try {
			$is_nonce_valid = check_ajax_referer( 'wc_youcanpay_save_upe_appearance_nonce', false, false );
			if ( ! $is_nonce_valid ) {
				throw new Exception(
					__( 'Unable to update UPE appearance values at this time.', 'woocommerce-gateway-youcanpay' )
				);
			}

			$appearance = isset( $_POST['appearance'] ) ? wc_clean( wp_unslash( $_POST['appearance'] ) ) : null;
			if ( null !== $appearance ) {
				set_transient( WC_YouCanPay_UPE_Payment_Gateway::UPE_APPEARANCE_TRANSIENT, $appearance, DAY_IN_SECONDS );
			}
	...
	public function clear_upe_appearance_transient() {
		delete_transient( WC_YouCanPay_UPE_Payment_Gateway::UPE_APPEARANCE_TRANSIENT );
	}
	...
	public function update_order_status_ajax() {
		try {
			$is_nonce_valid = check_ajax_referer( 'wc_youcanpay_update_order_status_nonce', false, false );
			if ( ! $is_nonce_valid ) {
				throw new WC_YouCanPay_Exception( 'missing-nonce', __( 'CSRF verification failed.', 'woocommerce-gateway-youcanpay' ) );
			}
			...
			$order    = wc_get_order( $order_id );
			if ( ! $order ) {
				throw new WC_YouCanPay_Exception( 'order_not_found', __( "We're not able to process this payment. Please try again later.", 'woocommerce-gateway-youcanpay' ) );
			}
			...
			if ( empty( $intent_id_received ) || $intent_id_received !== $intent_id ) {
				$note = sprintf(
					...
					esc_html__( 'A payment with ID %s was used in an attempt to pay for this order. This payment intent ID does not match any payments for this order, so it was ignored and the order was not updated.', 'woocommerce-gateway-youcanpay' ),
					$intent_id_received
				);
				$order->add_order_note( $note );
				throw new WC_YouCanPay_Exception( 'invalid_intent_id', __( "We're not able to process this payment. Please try again later.", 'woocommerce-gateway-youcanpay' ) );
			}
	...
	public function update_failed_order_ajax() {
		try {
			$is_nonce_valid = check_ajax_referer( 'wc_youcanpay_update_failed_order_nonce', false, false );
			if ( ! $is_nonce_valid ) {
				throw new WC_YouCanPay_Exception( 'missing-nonce', __( 'CSRF verification failed.', 'woocommerce-gateway-youcanpay' ) );
			}
			...
			if ( ! empty( $order_id ) && ! empty( $intent_id ) && is_object( $order ) ) {
				...
				if ( $payment_needed ) {
					$intent = WC_YouCanPay_API::retrieve( "payment_intents/$intent_id" );
				} else {
					$intent = WC_YouCanPay_API::retrieve( "setup_intents/$intent_id" );
				}
	...
	public function maybe_process_upe_redirect() {
		$gateway = $this->get_gateway();
		if ( is_a( $gateway, 'WC_YouCanPay_UPE_Payment_Gateway' ) ) {
			$gateway->maybe_process_upe_redirect();
		}
	}
}

new WC_YouCanPay_Intent_Controller();
