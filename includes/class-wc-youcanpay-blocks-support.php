 ...
 */
final class WC_YouCanPay_Blocks_Support extends AbstractPaymentMethodType {
	/**
	 * Payment method name defined by payment methods extending this class.
	 *
	 * @var string
	 */
	protected $name = 'youcanpay';
	 ...
	 * Constructor
	 *
	 * @param WC_YouCanPay_Payment_Request  The YouCanPay Payment Request configuration used for Payment
	 *                                   Request buttons.
	 */
	public function __construct( $payment_request_configuration = null ) {
		add_action( 'woocommerce_rest_checkout_process_payment_with_context', [ $this, 'add_payment_request_order_meta' ], 8, 2 );
		add_action( 'woocommerce_rest_checkout_process_payment_with_context', [ $this, 'add_youcanpay_intents' ], 9999, 2 );
		$this->payment_request_configuration = null !== $payment_request_configuration ? $payment_request_configuration : new WC_YouCanPay_Payment_Request();
	}
	...
	public function initialize() {
		$this->settings = get_option( 'woocommerce_youcanpay_settings', [] );
	}
	...
	public function get_payment_method_script_handles() {
		// Ensure YouCanPay JS is enqueued
		wp_register_script(
			'youcanpay',
		...
		return [ 'wc-youcanpay-blocks-integration' ];
	}
	...
	private function register_upe_payment_method_script_handles() {
		$asset_path   = WC_YOUCAN_PAY_PLUGIN_PATH . '/build/upe_blocks.asset.php';
		...
		wp_register_script(
			...
			WC_YOUCAN_PAY_PLUGIN_URL . '/build/upe_blocks.js',
			array_merge( [ 'youcanpay' ], $dependencies ),
			$version,
			true
		);
	...
	private function register_legacy_payment_method_script_handles() {
		$asset_path   = WC_YOUCAN_PAY_PLUGIN_PATH . '/build/index.asset.php';
		$version      = WC_YOUCAN_PAY_VERSION;
		$dependencies = [];
		if ( file_exists( $asset_path ) ) {
			$asset        = require $asset_path;
		...
		wp_register_script(
			'wc-youcanpay-blocks-integration',
			WC_YOUCAN_PAY_PLUGIN_URL . '/build/index.js',
			array_merge( [ 'youcanpay' ], $dependencies ),
			$version,
			true
		);
	...
	/**
	 * Returns the YouCanPay Payment Gateway JavaScript configuration object.
	 *
	 * @return array  the JS configuration from the YouCanPay Payment Gateway.
	 */
	private function get_gateway_javascript_params() {
		...
		if ( isset( $gateways['youcanpay'] ) ) {
			$js_configuration = $gateways['youcanpay']->javascript_params();
		}
	...
	/**
	 * Returns the YouCanPay Payment Request JavaScript configuration object.
	 *
	 * @return array  the JS configuration for YouCanPay Payment Requests.
	 */
	...
	public function add_payment_request_order_meta( PaymentContext $context, PaymentResult &$result ) {
		...
		if ( ! empty( $data['payment_request_type'] ) && 'youcanpay' === $context->payment_method ) {
			$this->add_order_meta( $context->order, $data['payment_request_type'] );
		}

		// hook into youcanpay error processing so that we can capture the error to
		// payment details (which is added to notices and thus not helpful for
		// this context).
		if ( 'youcanpay' === $context->payment_method ) {
			add_action(
				'wc_gateway_youcanpay_process_payment_error',
	...
	/**
	 * Handles any potential youcanpay intents on the order that need handled.
	 *
	 ...
	 * @param PaymentResult  $result  Result object for the payment.
	 */
	public function add_youcanpay_intents( PaymentContext $context, PaymentResult &$result ) {
		if ( 'youcanpay' === $context->payment_method
			&& (
			...
			$verification_endpoint = add_query_arg(
				...
				home_url() . \WC_Ajax::get_endpoint( 'wc_youcanpay_verify_intent' )
			);
