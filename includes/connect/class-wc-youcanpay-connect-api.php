...
if ( ! class_exists( 'WC_YouCanPay_Connect_API' ) ) {
	/**
	 * YouCan Pay Connect API class.
	 */
	class WC_YouCanPay_Connect_API {

		const WOOCOMMERCE_CONNECT_SERVER_API_VERSION = '3';

		/**
		 * Send request to Connect Server to initiate YouCan Pay OAuth
		 *
		 * @param  string $return_url return address.
		 *
		 * @return array
		 */
		public function get_youcanpay_oauth_init( $return_url ) {
			...
			return $this->request( 'POST', '/youcanpay/oauth-init', $request );
		}

		/**
		 * Send request to Connect Server for YouCan Pay keys
		 *
		 * @param  string $code OAuth server code.
		 *
		 * @return array
		 */
		public function get_youcanpay_oauth_keys( $code ) {

			$request = [ 'code' => $code ];

			return $this->request( 'POST', '/youcanpay/oauth-keys', $request );
		}
		...
		protected function request( $method, $path, $body = [] ) {
			...
			if ( ! is_array( $body ) ) {
				return new WP_Error(
					...
					__( 'Unable to send request to WooCommerce Connect server. Body must be an array.', 'woocommerce-gateway-youcanpay' )
				);
			}
			...
			if ( in_array( $method, [ 'POST', 'PUT' ], true ) ) {
				...
				if ( ! $body ) {
					return new WP_Error(
						...
						__( 'Unable to encode body for request to WooCommerce Connect server.', 'woocommerce-gateway-youcanpay' )
					);
				}
			}
				...
				if ( 200 !== $response_code ) {
					return new WP_Error(
						...
						sprintf(
							...
							__( 'Error: The WooCommerce Connect server returned HTTP code: %d', 'woocommerce-gateway-youcanpay' ),
							$response_code
						)
					);
				} else {
					return new WP_Error(
						...
						sprintf(
							...
							__( 'Error: The WooCommerce Connect server returned an invalid content-type: %s.', 'woocommerce-gateway-youcanpay' ),
							$content_type
						)
					);
				}
			...
			if ( 200 !== $response_code ) {
				if ( empty( $response_body ) ) {
					return new WP_Error(
						...
						sprintf(
							// Translators: HTTP error code.
							__( 'Error: The WooCommerce Connect server returned ( %d ) and an empty response body.', 'woocommerce-gateway-youcanpay' ),
							$response_code
						)
					);
				}
				...
				return new WP_Error(
					...
					sprintf(
						...
						__( 'Error: The WooCommerce Connect server returned: %1$s %2$s ( %3$d )', 'woocommerce-gateway-youcanpay' ),
						$error,
						$message,
						$response_code
					),
		...
		protected function request_body( $initial_body = [] ) {
			...
			$body['settings'] = wp_parse_args(
				...
				[
					...
					'youcanpay_version' => WC_YOUCAN_PAY_VERSION,
