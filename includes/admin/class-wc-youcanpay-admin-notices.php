		...
		if ( isset( $options['enabled'] ) && 'yes' === $options['enabled'] ) {
			if ( empty( $show_phpver_notice ) ) {
				if ( version_compare( phpversion(), WC_YOUCAN_PAY_MIN_PHP_VER, '<' ) ) {
					...
					$message = __( 'YouCan Pay - The minimum PHP version required for this plugin is %1$s. You are running %2$s.',
						'youcan-pay' );
			...
			if ( empty( $show_wcver_notice ) ) {
				if ( WC_YouCanPay_Helper::is_wc_lt( WC_YOUCAN_PAY_FUTURE_MIN_WC_VER ) ) {
					/* translators: 1) int version 2) int version */
					$message = __( 'YouCan Pay - This is the last version of the plugin compatible with WooCommerce %1$s. All future versions of the plugin will require WooCommerce %2$s or greater.',
						'youcan-pay' );
			...
			if ( empty( $show_keys_notice ) ) {
				...
				if ( $sandbox_mode ) {
					if (
						...
						|| ! empty( $sandbox_private_key ) && ! preg_match( '/^pri_sandbox_/',
							$sandbox_private_key ) ) {
						$setting_link = $this->get_setting_link();
						/* translators: 1) link */
						$this->add_admin_notice( 'keys',
