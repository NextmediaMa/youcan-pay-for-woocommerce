 ...
 * @since 5.5.0
 */
class WC_YouCanPay_Old_Settings_UPE_Toggle_Controller {
	protected $was_upe_checkout_enabled = null;

	public function __construct() {
		add_filter( 'pre_update_option_woocommerce_youcanpay_settings', [ $this, 'pre_options_save' ] );
		add_action( 'update_option_woocommerce_youcanpay_settings', [ $this, 'maybe_enqueue_script' ] );
	}
	...
	public function pre_options_save( $value ) {
		$this->was_upe_checkout_enabled = WC_YouCanPay_Feature_Flags::is_upe_checkout_enabled();
	...
	public function upe_toggle_script() {
		...
		$script_asset_path = WC_YOUCAN_PAY_PLUGIN_PATH . '/build/old_settings_upe_toggle.asset.php';
		...
		wp_register_script(
			'woocommerce_youcanpay_old_settings_upe_toggle',
			plugins_url( 'build/old_settings_upe_toggle.js', WC_YOUCAN_PAY_MAIN_FILE ),
		...
		wp_localize_script(
			'woocommerce_youcanpay_old_settings_upe_toggle',
			'wc_youcanpay_old_settings_param',
			[
				'was_upe_enabled' => $this->was_upe_checkout_enabled,
				'is_upe_enabled'  => WC_YouCanPay_Feature_Flags::is_upe_checkout_enabled(),
			]
		);
		wp_enqueue_script( 'woocommerce_youcanpay_old_settings_upe_toggle' );
	}
}
