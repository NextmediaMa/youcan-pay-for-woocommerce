...
class WC_YouCanPay_Privacy extends WC_Abstract_Privacy {
	...
	public function __construct() {
		...
		$this->add_exporter(
			'youcan-pay-customer-data',
			__( 'YouCan Pay Customer Data', 'youcan-pay' ),
	...
	public function account_settings( $settings ) {
		$insert_setting = [
			[
				'title'       => __( 'Retain YouCan Pay Data', 'youcan-pay' ),
		...
		if ( 0 < count( $orders ) ) {
			foreach ( $orders as $order ) {
				$data_to_export[] = [
					...
					'data'        => [
						[
							'name'  => __( 'YouCan Pay payment id', 'youcan-pay' ),
							'value' => get_post_meta( $order->get_id(), '_youcanpay_source_id', true ),
						],
						[
							'name'  => __( 'YouCan Pay customer id', 'youcan-pay' ),
	...
	public function customer_data_exporter( $email_address, $page ) {
		...
		if ( $user instanceof WP_User ) {
			$data_to_export[] = [
				...
				'group_label' => __( 'Customer Data', 'youcan-pay' ),
				'item_id'     => 'user',
				'data'        => [
					[
						'name'  => __( 'YouCan Pay payment id', 'youcan-pay' ),
	...
	public function customer_data_eraser( $email_address, $page ) {
		...
		if ( ! empty( $youcanpay_customer_id ) || ! empty( $youcanpay_source_id ) ) {
			...
			$messages[] = __( 'YouCan Pay User Data Erased.', 'youcan-pay' );
		}
