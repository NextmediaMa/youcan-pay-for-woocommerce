<?php
/**
 * YouCanPay Apple Pay Registration Class.
 *
 * @since 4.0.6
 */
...
class WC_YouCanPay_Apple_Pay_Registration {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_youcanpay_updated', [ $this, 'verify_domain_if_configured' ] );
		add_action( 'add_option_woocommerce_youcanpay_settings', [ $this, 'verify_domain_on_new_settings' ], 10, 2 );
		add_action( 'update_option_woocommerce_youcanpay_settings', [ $this, 'verify_domain_on_updated_settings' ], 10, 2 );
	...
	public function get_option( $setting = '', $default = '' ) {
		if ( empty( $this->youcanpay_settings ) ) {
			return $default;
		}

		if ( ! empty( $this->youcanpay_settings[ $setting ] ) ) {
			return $this->youcanpay_settings[ $setting ];
		}
	...
	/**
	 * Gets the YouCanPay secret key for the current mode.
	 *
	 * @since 4.5.3
	 * @version 4.9.0
	...
	private function verify_hosted_domain_association_file_is_up_to_date() {
		...
		$new_contents = @file_get_contents( WC_YOUCAN_PAY_PLUGIN_PATH . '/' . self::DOMAIN_ASSOCIATION_FILE_NAME ); // @codingStandardsIgnoreLine
	...
	public function parse_domain_association_request( $wp ) {
		...
		$path = WC_YOUCAN_PAY_PLUGIN_PATH . '/' . self::DOMAIN_ASSOCIATION_FILE_NAME;
	...
	/**
	 * Makes request to register the domain with YouCanPay/Apple Pay.
	 *
	 * @since 3.1.0
	 * @version 4.9.0
	...
	private function make_domain_registration_request( $secret_key ) {
		...
		$headers = [
			'User-Agent'    => 'WooCommerce YouCanPay Apple Pay',
	...
	public function register_domain_with_apple( $secret_key ) {
		try {
			...
			$this->youcanpay_settings['apple_pay_verified_domain'] = $this->domain_name;
			$this->youcanpay_settings['apple_pay_domain_set']      = 'yes';
			$this->apple_pay_domain_set                         = true;

			update_option( 'woocommerce_youcanpay_settings', $this->youcanpay_settings );

			WC_YouCanPay_Logger::log( 'Your domain has been verified with Apple Pay!' );

			return true;

		} catch ( Exception $e ) {
			$this->youcanpay_settings['apple_pay_verified_domain'] = $this->domain_name;
			$this->youcanpay_settings['apple_pay_domain_set']      = 'no';
	...
	public function verify_domain_if_configured() {
		...
		WC_YouCanPay_Inbox_Notes::notify_on_apple_pay_domain_verification( $verification_complete );
	}
	...
	public function verify_domain_on_updated_settings( $prev_settings, $settings ) {
		...
		$this->youcanpay_settings = $prev_settings;
		$prev_secret_key       = $this->get_secret_key();
		$prev_is_enabled       = $this->is_enabled();
		$this->youcanpay_settings = $settings;

		// If YouCanPay or Payment Request Button wasn't enabled (or secret key was different) then might need to verify now.
	...
	public function admin_notices() {
		...
		?>
		<div class="error youcanpay-apple-pay-message">
			<?php if ( $empty_notice ) : ?>
...
new WC_YouCanPay_Apple_Pay_Registration();
