 ...
 * @since 4.0.0
 */
class WC_YouCanPay_Order_Handler extends WC_YouCanPay_Payment_Gateway {
	...
	public function process_redirect_payment( $order_id, $retry = true, $previous_error = false ) {
		try {
			...
			WC_YouCanPay_Logger::log( "Info: (Redirect) Begin processing payment for order $order_id for the amount of {$order->get_total()}" );

			/**
			 * First check if the source is chargeable at this time. If not,
			 * webhook will take care of it later.
			 */
			$source_info = WC_YouCanPay_API::retrieve( 'sources/' . $source );

			if ( ! empty( $source_info->error ) ) {
				throw new WC_YouCanPay_Exception( print_r( $source_info, true ), $source_info->error->message );
			}

			if ( 'failed' === $source_info->status || 'canceled' === $source_info->status ) {
				throw new WC_YouCanPay_Exception( print_r( $source_info, true ), __( 'Unable to process this payment, please try again or use alternative method.', 'woocommerce-youcan-pay' ) );
			}
			...
			if ( ! empty( $response->error ) ) {
				...
				throw new WC_YouCanPay_Exception( print_r( $response, true ), $message );
			}
			...
			do_action( 'wc_gateway_youcanpay_process_redirect_payment', $response, $order );
			...
			$order->update_status( 'failed', sprintf( __( 'YouCan Pay payment failed: %s', 'woocommerce-youcan-pay' ), $e->getLocalizedMessage() ) );
			...
			if ( $charge && 'no' === $captured ) {
				...
				if ( $intent ) {
					...
					if ( ! empty( $intent->error ) ) {
						/* translators: error message */
						$order->add_order_note( sprintf( __( 'Unable to capture charge! %s', 'woocommerce-youcan-pay' ), $intent->error->message ) );
					} elseif ( 'requires_capture' === $intent->status ) {
						$level3_data = $this->get_level3_data_from_order( $order );
						$result      = WC_YouCanPay_API::request_with_level3_data(
							[
								'amount'   => WC_YouCanPay_Helper::get_youcanpay_amount( $order_total ),
						...
						if ( ! empty( $result->error ) ) {
							/* translators: error message */
							$order->update_status( 'failed', sprintf( __( 'Unable to capture charge! %s', 'woocommerce-youcan-pay' ), $result->error->message ) );
						} else {
							$is_youcanpay_captured = true;
					...
					$result = WC_YouCanPay_API::retrieve( 'charges/' . $charge );

					if ( ! empty( $result->error ) ) {
						/* translators: error message */
						$order->add_order_note( sprintf( __( 'Unable to capture charge! %s', 'woocommerce-youcan-pay' ), $result->error->message ) );
				...
				if ( $is_youcanpay_captured ) {
					/* translators: transaction id */
					$order->add_order_note( sprintf( __( 'YouCanPay charge complete (Charge ID: %s)', 'woocommerce-youcan-pay' ), $result->id ) );
				...
				do_action( 'woocommerce_youcanpay_process_manual_capture', $order, $result );
			}
		}
	}
	...
	public function cancel_payment( $order_id ) {
		...
		if ( 'youcanpay' === $order->get_payment_method() ) {
			...
			do_action( 'woocommerce_youcanpay_process_manual_cancel', $order );
		}
	}
	...
	public function woocommerce_tracks_event_properties( $properties, $prefixed_event_name ) {
		...
		// Not youcanpay? Bail.
		if ( 'youcanpay' != $properties['payment_method'] ) {
			return $properties;
		}
		...
		if ( array_key_exists( 'testmode', $youcanpay_settings ) ) {
			$is_live = 'no' === $youcanpay_settings['testmode'];
		}

		$properties['admin_email']                        = get_option( 'admin_email' );
		$properties['is_live']                            = $is_live;
		$properties['woocommerce_gateway_youcanpay_version'] = WC_YOUCAN_PAY_VERSION;
...
new WC_YouCanPay_Order_Handler();
