 ...
 * @since 4.0.0
 */
class WC_YouCanPay_Payment_Tokens {
	...
	public static function customer_has_saved_methods( $customer_id ) {
		$gateways = [ 'youcanpay', 'youcanpay_sepa' ];
	...
	/**
	 * Gets saved tokens from YouCanPay, if they don't already exist in WooCommerce.
	 *
	...
	public function woocommerce_get_customer_payment_tokens_legacy( $tokens, $customer_id, $gateway_id ) {
		if ( is_user_logged_in() && class_exists( 'WC_Payment_Token_CC' ) ) {
			...
			if ( 'youcanpay' === $gateway_id ) {
				$youcanpay_customer = new WC_YouCanPay_Customer( $customer_id );
				$youcanpay_sources  = $youcanpay_customer->get_sources();

				foreach ( $youcanpay_sources as $source ) {
					if ( isset( $source->type ) && 'card' === $source->type ) {
						if ( ! isset( $stored_tokens[ $source->id ] ) ) {
							$token = new WC_Payment_Token_CC();
							$token->set_token( $source->id );
							$token->set_gateway_id( 'youcanpay' );
				...
				foreach ( $youcanpay_sources as $source ) {
					if ( isset( $source->type ) && 'sepa_debit' === $source->type ) {
						if ( ! isset( $stored_tokens[ $source->id ] ) ) {
							...
							$token->set_token( $source->id );
							$token->set_gateway_id( 'youcanpay_sepa' );
	...
	public function woocommerce_get_customer_upe_payment_tokens( $tokens, $user_id, $gateway_id ) {
		if ( ( ! empty( $gateway_id ) && WC_YouCanPay_UPE_Payment_Gateway::ID !== $gateway_id ) || ! is_user_logged_in() ) {
			return $tokens;
		}
		...
		if ( count( $tokens ) >= get_option( 'posts_per_page' ) ) {
			...
			// Having 10 saved credit cards is considered an unsupported edge case, new ones that have been stored in YouCanPay won't be added.
			return $tokens;
		}

		$gateway                  = new WC_YouCanPay_UPE_Payment_Gateway();
		...
		foreach ( $tokens as $token ) {
			if ( WC_YouCanPay_UPE_Payment_Gateway::ID === $token->get_gateway_id() ) {
				...
				if ( ! in_array( $payment_method_type, $reusable_payment_methods, true ) ) {
					...
					// We will use this list to check whether these methods still exist on YouCanPay's side.
		...
		foreach ( $retrievable_payment_method_types as $payment_method_id ) {
			...
			foreach ( $payment_methods as $payment_method ) {
				if ( ! isset( $remaining_tokens[ $payment_method->id ] ) ) {
					...
					// Count that existing token for payment method is still present on YouCanPay.
					// Remaining IDs in $remaining_tokens no longer exist with YouCanPay and will be eliminated.
		...
		// Eliminate remaining payment methods no longer known by YouCanPay.
	...
	/**
	 * Returns original type of payment method from YouCanPay payment method response,
	 * after checking whether payment method is SEPA method generated from another type.
	 *
	 * @param object $payment_method YouCanPay payment method JSON object.
	 *
	 * @return string Payment method type/ID
	 */
	private function get_original_payment_method_type( $payment_method ) {
		if ( WC_YouCanPay_UPE_Payment_Method_Sepa::YOUCAN_PAY_ID === $payment_method->type ) {
	...
	/**
	 * Returns original YouCanPay payment method type from payment token
	 *
	...
	public function woocommerce_payment_token_deleted( $token_id, $token ) {
		...
		if ( WC_YouCanPay_Feature_Flags::is_upe_checkout_enabled() ) {
			if ( WC_YouCanPay_UPE_Payment_Gateway::ID === $token->get_gateway_id() ) {
				$youcanpay_customer->detach_payment_method( $token->get_token() );
			}
		} else {
			if ( 'youcanpay' === $token->get_gateway_id() || 'youcanpay_sepa' === $token->get_gateway_id() ) {
				$youcanpay_customer->delete_source( $token->get_token() );
			}
		}
	}

	/**
	 * Set as default in YouCanPay.
	 *
	 * @since 3.1.0
	 * @version 4.0.0
	 */
	public function woocommerce_payment_token_set_default( $token_id ) {
		...
		if ( WC_YouCanPay_Feature_Flags::is_upe_checkout_enabled() ) {
			if ( WC_YouCanPay_UPE_Payment_Gateway::ID === $token->get_gateway_id() ) {
				$youcanpay_customer->set_default_payment_method( $token->get_token() );
			}
		} else {
			if ( 'youcanpay' === $token->get_gateway_id() || 'youcanpay_sepa' === $token->get_gateway_id() ) {
				$youcanpay_customer->set_default_source( $token->get_token() );
			}
		}
	}
}

new WC_YouCanPay_Payment_Tokens();
