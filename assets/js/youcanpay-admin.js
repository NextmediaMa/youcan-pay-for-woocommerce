	...
	var wc_youcanpay_admin = {
		...
		getSecretKey: function() {
			if ( wc_youcanpay_admin.isSandboxMode() ) {
				...
				return $( '#woocommerce_youcanpay_production_private_key' ).val();
			}
		},
		...
		init: function() {
			$( document.body ).on( 'change', '#woocommerce_youcanpay_sandbox_mode', function() {
				var sandbox_private_key = $( '#woocommerce_youcanpay_sandbox_private_key' ).parents( 'tr' ).eq( 0 ),
					...
					production_private_key = $( '#woocommerce_youcanpay_production_private_key' ).parents( 'tr' ).eq( 0 ),
					production_public_key = $( '#woocommerce_youcanpay_production_public_key' ).parents( 'tr' ).eq( 0 );
